...
binCItest <- function (x, y, S, suffStat) {
    ...
    gSquareBin(x = x, y = y, S = S, dm = dm, adaptDF = adaptDF, n.min=-1, 
        verbose = FALSE)
}

gSquareBin <- function (x, y, S, dm, adaptDF = FALSE, n.min = 10 * df, verbose = FALSE) {
  ## I changed the following so that the algorithm does not stop if there are
  ## zero observations, but just returns NA
  n <- nrow(dm)
  if (n==0) {return(NA)}
                      ...
                      for (i5 in n12) {
                        ...
                        for (i6 in n12) {
                          d.xy.S1S2S3S4 <- d.xy.S1S2S3 & dmS4.1 == 
                            i6
                          for (i7 in n12) nijk[i1, i2, 16 * (i3 - 
                            1) + 8 * (i4 - 1) + 4 * (i5 - 1) + 
                            2 * (i6 - 1) + i7] <- sum(d.xy.S1S2S3S4 & 
                            dmS5.1 == i7)
                        }
                      }
                    }
    ...
    if (adaptDF && lenS > 0) {
        ...
        if (verbose) 
            cat("adaptDF: (df=", df, ", zero.counts=", 
                zero.counts, ") ==> new df = ", ndf, "\n", 
                sep = "")
        df <- ndf
    }
