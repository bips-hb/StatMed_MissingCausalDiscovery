...
trueMoral <-
  trueMoral[c("b1191", "cchB", "eutG", "fixC", "ibpB", "sucA", "tnaA", "yceP",
              "yfaD", "ygbD", "ygcE", "yjbO"),
            c("b1191", "cchB", "eutG", "fixC", "ibpB", "sucA", "tnaA", "yceP",
              "yfaD", "ygbD", "ygcE", "yjbO")]
...
sim <- function(., network, trueMoral, n, mech) {
  ...
  ### generate data ############################################################
  dat <- rbn(network, n=n)
  dat <- dat[ ,c("b1191", "cchB", "eutG", "fixC", "ibpB", "sucA", "tnaA",
  ...
  tu1_res <- tryCatch( mvpc(suffStat=list(data=dat), indepTest=gaussCItest.td,
                            corrMethod=gaussCItest.permc, alpha=0.05, p=12,
                            labels=colnames(dat), skel.method="stable",
                            maj.rule=TRUE, solve.confl=TRUE),
                       error=function(e){paste("Error")} )
  ...
  tu2_res <- tryCatch( mvpc(suffStat=list(data=dat), indepTest=gaussCItest.td,
                            corrMethod=gaussCItest.drw, alpha=0.05, p=12,
                            labels=colnames(dat), skel.method="stable",
                            maj.rule=TRUE, solve.confl=TRUE),
                       error=function(e){paste("Error")} )
...
start <- Sys.time()
ECOLI_100_MCAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
                                 trueMoral=trueMoral, n=100, mech="MCAR")
end <- Sys.time()
end-start

save(list=ls()[grepl("ECOLI", ls())],
     file="res_ECOLI.RData")

start <- Sys.time()
ECOLI_100_MAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_100_MNAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_1000_MCAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_1000_MAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_1000_MNAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_5000_MCAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_5000_MAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
...
start <- Sys.time()
ECOLI_5000_MNAR <- parLapply(cl, 1:nrep, sim, network=ecoli,
